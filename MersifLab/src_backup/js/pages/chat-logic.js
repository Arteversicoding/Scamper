...
// Function to check and load PDF context from Supabase documents table
// async function checkForPdfContext() {
//     // First, try to get the document from Supabase
//     try {
//         // Get document_id from URL, sessionStorage, or fetch the latest document
//         const urlParams = new URLSearchParams(window.location.search);
//         let documentId = urlParams.get('document_id');
        
//         if (!documentId) {
//             // Try to get from sessionStorage first
//             const context = sessionStorage.getItem('pdfContext');
//             if (context) {
//                 const parsedContext = JSON.parse(context);
//                 if (parsedContext.id) {
//                     documentId = parsedContext.id;
//                 }
//             }
//         }
...
//         if (documentId) {
//             // If we have an ID, fetch that specific document
//             console.log('Mengambil dokumen spesifik dari Supabase...');
...
//             error = result.error;
//         } else {
//             // If no ID, fetch the most recently created document
//             console.log('Mengambil dokumen terbaru dari Supabase...');
...
//                 console.log('Tidak ada dokumen yang tersedia di database');
//                 return false;
//             }
            
//             document = data[0];
//             console.log('Dokumen ditemukan:', document);
//         }
        
//         if (!document) {
//             console.error('Dokumen tidak ditemukan di database');
//             return false;
//         }

//         if (!document.text_content) {
//             console.error('Dokumen tidak memiliki konten teks');
//             return false;
//         }

//         console.log(`Dokumen berhasil diambil: ${document.file_name}`);
        
//         // Save the document data to sessionStorage
//         const pdfData = {
//             id: documentId,
...
//         sessionStorage.setItem('pdfContext', JSON.stringify(pdfData));
        
//         // Show notification
//         const notification = document.createElement('div');
//         notification.className = 'bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-4 rounded';
//         notification.innerHTML = `
//             <div class="flex items-center">
//                 <i class="fas fa-file-pdf text-blue-500 mr-2"></i>
//                 <span>Dokumen <strong>${document.file_name}</strong> siap digunakan untuk konsultasi.</span>
//             </div>
//         `;
...
//     } catch (error) {
//         console.error('Gagal mengambil dokumen dari Supabase:', error);
        
//         // Fallback to sessionStorage if available
//         const pdfContext = sessionStorage.getItem('pdfContext');
//         if (pdfContext) {
//             try {
//                 const context = JSON.parse(pdfContext);
//                 if (context.text) {
//                     console.log('Menggunakan teks dokumen dari sessionStorage');
//                     return true;
//                 }
//             } catch (e) {
//                 console.error('Error memproses konteks PDF dari sessionStorage:', e);
//             }
//         }
        
//         console.log('Tidak ada data dokumen yang valid');
//         return false;
//     }
// }
let chatHistory = [];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Periksa dan muat konteks PDF jika ada
    // checkForPdfContext();
...
Pertanyaan: ${prompt}
        ...
        // Jangan simpan ke chatHistory karena konteks selalu fresh dari database

    } catch (error) {
        console.error('❌ Error:', error);
        updateMessage(loadingMsgId, `⚠️ ${error.message || "Gagal memproses permintaan. Pastikan ada dokumen di tabel 'documents' dengan kolom 'text_content'."}`);
        scrollToBottom();
    }
}
